...
- (void)loadMetal
{
    ...
    // the uniform and light position buffers and put them into CArrays.  Then iniilize the iVars
    // for the NSArray properties with the CArrays.
...
#if USE_SINGLE_PASS_DEFERRED_LIGHTING
...
- (nonnull id <MTLCommandBuffer>)beginDrawableCommands
{
    ...
    // Add completion hander which signal _inFlightSemaphore when Metal and the GPU has fully
    //   finished processing the commands encoded for this frame.  This indicates when the
    //   dynamic buffers, written this frame, will no longer be needed by Metal and the GPU.
    __block dispatch_semaphore_t block_sema = _inFlightSemaphore;
    [commandBuffer addCompletedHandler:^(id<MTLCommandBuffer> buffer)
     {
         dispatch_semaphore_signal(block_sema);
